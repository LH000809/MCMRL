...
class GIN(nn.Module):
    ...
    def __init__(self, 
        ...
        # Fingerprint encoder - output feat_dim // 2 (256) to match pretrained model
        self.fp_dim = 1489
        self.fp_encoder = nn.Sequential(
            ...
            nn.Linear(1024, 512),
            nn.ReLU(),
            nn.Linear(512, self.feat_dim // 2)  # Output feat_dim // 2 (256) to match pretrained
        )
        ...
        # Prediction head accepts concatenated features: graph features (feat_dim) + fp features (feat_dim // 2)
        combined_dim = self.feat_dim // 2 * 3  # 512 + 256 = 768
        
        if pred_act == 'relu':
            pred_head = [
                nn.Linear(combined_dim, self.feat_dim), 
