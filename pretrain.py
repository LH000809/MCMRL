...
class MolCLR(object):
    ...
    def train(self):
        ...
        if self.config['model_type'] == 'gin':
            ...
            from models.gcn_pretrain import GCN
            model = GCN(**self.config["model"]).to(self.device)
            model = self._load_pre_trained_weights(model)
        elif self.config['model_type'] == 'gat':
            from models.gat_pretrain import GAT
            model = GAT(**self.config["model"]).to(self.device)
            model = self._load_pre_trained_weights(model)
        else:
...
def main():
    ...
    elif config['aug'] == 'lmdb':
        from dataset.dataset_lmdb import MoleculeDatasetWrapper
    else:
