...
def _save_config_file(model_checkpoints_folder, config_path='config_finetune.yaml'):
    if not os.path.exists(model_checkpoints_folder):
        os.makedirs(model_checkpoints_folder)
        shutil.copy(config_path, os.path.join(model_checkpoints_folder, 'config_finetune.yaml'))
...
class FineTune(object):
    ...
    def _step(self, model, data, datafp, n_iter):
        # get the prediction
        __, pred = model(data, datafp)  # [N,C]
    ...
    def train(self):
        ...
        # save config file
        _save_config_file(model_checkpoints_folder, args.config)
        ...
        for epoch_counter in range(self.config['epochs']):
            for bn, (data, datafp) in enumerate(train_loader):
                optimizer.zero_grad()

                data = data.to(self.device)
                datafp = datafp.to(self.device)
                loss = self._step(model, data, datafp, n_iter)
    ...
    def _validate(self, model, valid_loader):
        ...
        with torch.no_grad():
            ...
            valid_loss = 0.0
            num_data = 0
            for bn, (data, datafp) in enumerate(valid_loader):
                data = data.to(self.device)
                datafp = datafp.to(self.device)
                __, pred = model(data, datafp)
                loss = self._step(model, data, datafp, bn)
    ...
    def _test(self, model, test_loader):
        ...
        with torch.no_grad():
            ...
            test_loss = 0.0
            num_data = 0
            for bn, (data, datafp) in enumerate(test_loader):
                data = data.to(self.device)
                datafp = datafp.to(self.device)
                __, pred = model(data, datafp)
                loss = self._step(model, data, datafp, bn)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Fine-tune MGNN')
    ...
    elif config["task_name"] == 'DRD1':
        config['dataset']['task'] = 'regression'
        config['dataset']['data_path'] = 'with_negative/DRD1.csv'
        target_list = ['pXC50']
    elif config["task_name"] == 'DRD2':
        config['dataset']['task'] = 'regression'
        config['dataset']['data_path'] = 'with_negative/DRD2.csv'
        target_list = ['pXC50']
    elif config["task_name"] == 'DRD3':
        config['dataset']['task'] = 'regression'
        config['dataset']['data_path'] = 'with_negative/DRD3.csv'
        target_list = ['pXC50']
    elif config["task_name"] == 'DRD4':
        config['dataset']['task'] = 'regression'
        config['dataset']['data_path'] = 'with_negative/DRD4.csv'
        target_list = ['pXC50']
    elif config["task_name"] == 'DRD5':
        config['dataset']['task'] = 'regression'
        config['dataset']['data_path'] = 'with_negative/DRD5.csv'
        target_list = ['pXC50']
