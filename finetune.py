...
from sklearn.metrics import roc_auc_score, mean_squared_error, mean_absolute_error, mean_squared_error
...
class FineTune(object):
    ...
    def _step(self, model, data, n_iter):
        # get the prediction
        # data is a tuple of (graph_data, fp_data)
        graph_data, fp_data = data
        __, pred = model(graph_data, fp_data)  # [N,C]

        if self.config['dataset']['task'] == 'classification':
            loss = self.criterion(pred, graph_data.y.flatten())
        elif self.config['dataset']['task'] == 'regression':
            if self.normalizer:
                loss = self.criterion(pred, self.normalizer.norm(graph_data.y))
            else:
                loss = self.criterion(pred, graph_data.y)
    ...
    def train(self):
        ...
        if self.config["task_name"] in ['qm7', 'qm9']:
            labels = []
            for graph_data_batch, fp_data_batch in train_loader:
                # graph_data_batch is a Batch object containing multiple graphs
                labels.append(graph_data_batch.y)
            labels = torch.cat(labels)
            self.normalizer = Normalizer(labels)
            print(self.normalizer.mean, self.normalizer.std, labels.shape)

        if self.config['model_type'] == 'gin':
            from models.gin_finetune import GIN
            model = GIN(self.config['dataset']['task'], **self.config["model"]).to(self.device)
        ...
        for epoch_counter in range(self.config['epochs']):
            for bn, data in enumerate(train_loader):
                ...
                # data is a list of [graph_data, fp_data] from dataset
                graph_data, fp_data = data
                graph_data = graph_data.to(self.device)
                fp_data = fp_data.to(self.device)
                loss = self._step(model, (graph_data, fp_data), n_iter)
    ...
    def _validate(self, model, valid_loader):
        ...
        with torch.no_grad():
            ...
            for bn, data in enumerate(valid_loader):
                ...
                __, pred = model(graph_data, fp_data)
                loss = self._step(model, (graph_data, fp_data), bn)
                ...
                if self.device == 'cpu':
                    ...
                    labels.extend(graph_data.y.flatten().numpy())
                else:
...
if __name__ == "__main__":
    ...
    if config['task_name'] == 'BBBP':
        ...
        target_list = ['alpha'] # ,'homo' , 'lumo','gap' ,'r2', 'mu' , 'zpve', 'cv'

    elif config["task_name"] == 'herg_r':
        config['dataset']['task'] = 'regression'
        config['dataset']['data_path'] = 'data/herg/hERG_regression.csv'
        target_list = ['pIC50']

    elif config["task_name"] == 'herg_c':
        config['dataset']['task'] = 'classification'
        config['dataset']['data_path'] = 'data/herg/hERG_classification.csv'
        target_list = ['label']
    elif config["task_name"] == 'herg':
        config['dataset']['task'] = 'regression'
        config['dataset']['data_path'] = 'data/herg.csv'
        target_list = ['herg']
    else:
